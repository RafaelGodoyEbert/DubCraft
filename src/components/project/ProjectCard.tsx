...
export const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  const percent = project.totalLines > 0 ? Math.round((project.reviewedLines / project.totalLines) * 100) : 0;
  ...
  return (
    <div
      ...
      className={`group bg-zinc-900 border rounded-2xl p-4 shadow-lg hover:shadow-2xl transition-all cursor-pointer flex flex-col justify-between overflow-hidden relative ${
        isCompleted
        ...
        <div className="absolute bottom-2 left-3 right-3 flex items-center justify-between">
          {isCompleted ? (
            <span className="text-xs font-bold px-2.5 py-1 bg-emerald-500 text-zinc-950 rounded-full shadow flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5" /> Dublagem Pronta
            </span>
          ) : isPaused ? (
          ...
          <span className="text-xs font-medium px-2 py-0.5 bg-black/70 text-zinc-300 rounded border border-zinc-700/60 backdrop-blur-sm flex items-center gap-1">
            <Film className="w-3 h-3" /> {project.cutscenesCount} Cutscenes/Mapas
          </span>
        </div>
      </div>
